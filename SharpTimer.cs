        ...
        public override void Load(bool hotReload)
        {
            ...
            HookEntityOutput("trigger_multiple", "OnStartTouch", (CEntityIOOutput output, string name, CEntityInstance activator, CEntityInstance caller, CVariant value, float delay) =>
            {
                try
                {
                    ...
                    if (((useStageTriggers == true && stageTriggers.ContainsKey(caller.Handle) && stageTriggers[caller.Handle] != 1) || (useStageTriggers == false && cpTriggers.ContainsKey(caller.Handle))) && playerTimers[player.Slot].IsTimerBlocked == false && playerTimers[player.Slot].IsTimerRunning == true && IsAllowedPlayer(player))
                    {
                        HandlePlayerStageTimes(player, caller.Handle);
                    }
                    else if ((stageTriggers.ContainsKey(caller.Handle) && stageTriggers[caller.Handle] == 1) ||(cpTriggers.ContainsKey(caller.Handle) && cpTriggers[caller.Handle] == 1) )
                    {
